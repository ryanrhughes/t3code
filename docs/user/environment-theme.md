...
```bash
t3 theme set omarchy
```

Every client switches to it — immediately when connected, on its next connect otherwise — and a
fresh client opens with it. Each client applies a set theme once, so picking a different theme in
Settings afterwards sticks until the next `t3 theme set`. `t3 theme clear` removes the setting
without changing what anyone currently has. Only the environment you are anchored to publishes
themes, so a remote client follows the machine it is connected to, not the device it runs on.
...
directory (`~/.t3/userdata/themes/` by default). The filename is the theme id — `omarchy.json`
appears as `omarchy` — and stays stable while the machine rewrites the colors underneath, so
selections and defaults keep pointing at it.

Two formats are accepted. A theme exported from T3 Code (the **Download** button's output) works
as-is, so any theme someone shared can be dropped in unchanged. Or, for a desktop that only knows
